...
#define gpuErrchk(ans)                        \
    ...
    __device__ bool checkNeighbour(int gid, int neighIndex, const BoidData boidData)
    {
        if (gid == neighIndex)
            return false;
    ...
    __device__ bool checkSpecies(int gid, int neighIndex, const BoidData boidData)
    {
    ...
    __device__ bool checkNeighbourGrid(glm::vec2 boidPosition, glm::vec2 neighPosition, const BoidData boidData)
    {
    ...
    __device__ glm::vec2 steeringForce(glm::vec2 target, glm::vec2 velocity, const BoidData boidData)
    {
    ...
    __device__ glm::vec2 alignmentForceGrid(int gid, int boidsCount, const BoidData boidData)
    {
    ...
    __device__ glm::vec2 cohesionForceGrid(int gid, int boidsCount, const BoidData boidData)
    {
        glm::vec2 target = glm::vec2(0, 0);
        int neighsCount = 0;
    ...
    __device__ glm::vec2 separationForceGrid(const int gid, const int boidsCount, BoidData boidData)
    {
        glm::vec2 target = glm::vec2(0, 0);
        glm::vec2 value2 = glm::vec2(0, 0);// declaring it here is fine
        int neighsCount = 0;
        ...
        for (int neighCell : neighCells)
        {
            ...
            //printf("killed cell: %d with neighCell: %d\n",cell, neighCell);
            for (int i = boidData.device_gridCellStart[neighCell]; i <= boidData.device_gridCellEnd[neighCell]; i++)
            {
                //printf("i: %d, neighCell: %d\n", i, neighCell);
                if (!checkNeighbourGrid(boidData.device_positions[gid], boidData.device_positionsSorted[i], boidData))
                    continue;
                glm::vec2 offset = boidData.device_positions[gid] - boidData.device_positionsSorted[i];
                if (length(offset) == 0)
    ...
    __device__ void applyForces(int gid, int boidsCount, BoidData boidData)
    {
        ...
        //boidData.device_accelerations[gid] += boidData.params.alignmentForce * alignmentForceGrid(gid, boidsCount, boidData);
        //boidData.device_accelerations[gid] += boidData.params.cohesionForce * (cohesionForceGrid(gid, boidsCount, boidData));
        //boidData.device_accelerations[gid] += boidData.params.separationForce * (separationForce(gid, boidsCount, boidData));
        //boidData.device_accelerations[gid] += 5.0f * (cursorDodgeForce(gid, boidsCount, boidData));
    }
